...
export const RoleBasedRoute = ({
  ...
  const { isAuthenticated, profile, isLoading, isProfileLoading } = useAuth();
  const location = useLocation();
  ...
  // Wait until profile loads completely
  if (isProfileLoading || !profile) {
    return <Loading message="Loading profile..." fullScreen />;
  }
  ...
  const isManager =
    profileRole === "manager" && allowedRoles.includes(Role.MANAGER);

  const isSalesRep =
    (profileRole === "sales_representative" ||
      profileRole === "salesrepresentative") &&
    allowedRoles.includes(Role.SALES_REPRESENTATIVE);

  if (!isManager && !isSalesRep) {
    return <Navigate to={ROUTES.HOME} replace />;
  }
